...
BUNDLE_NAME = mcp-git-worktree
VERSION ?= 0.1.0

.PHONY: help install dev-install format format-check lint lint-fix typecheck test test-cov clean check all run run-stdio run-http test-http bump
...
test-cov: ## Run tests with coverage
	uv run pytest tests/ -v --cov=src/mcp_git_worktree --cov-report=term-missing
...
run: ## Run the MCP server
	uv run python -m mcp_git_worktree.server

run-stdio: ## Run in stdio mode (for Claude desktop)
	uv run fastmcp run src/mcp_git_worktree/server.py

run-http: ## Run HTTP server with uvicorn
	uv run uvicorn mcp_git_worktree.server:app --host 0.0.0.0 --port 8000
...
bump: ## Bump version across all files (usage: make bump VERSION=0.2.0)
	...
	@sed -i '' 's/^__version__ = ".*"/__version__ = "$(VERSION)"/' src/mcp_git_worktree/__init__.py
	@echo "Updated:"
	@echo "  manifest.json:                  $$(jq -r .version manifest.json)"
	@echo "  server.json:                    $$(jq -r .version server.json)"
	@echo "  pyproject.toml:                 $$(grep '^version' pyproject.toml)"
	@echo "  src/mcp_git_worktree/__init__.py: $$(grep '__version__' src/mcp_git_worktree/__init__.py)"
